...
export class SimpleWeatherPlatform implements DynamicPlatformPlugin {
  ...
  private getSimpleWeatherConfig(): SimpleWeatherConfig {
    return {
      ...
      todayDevices: {
        currentTemp: this.config.currentTemp ?? false,
        minTemp: this.config.minTemp ?? false,
        maxTemp: this.config.maxTemp ?? false,
        humidity: this.config.humidity ?? false,
        rainProb: false,
      },
      forecastNum: this.config.forecastNum ?? 0,
      forecastInterval: this.config.forecastInterval ?? 0,
      forecastDevices: {
        currentTemp: this.config.forecastCurrentTemp ?? false,
        minTemp: this.config.forecastMinTemp ?? false,
        maxTemp: this.config.forecastMaxTemp ?? false,
        humidity: this.config.forecastHumidity ?? false,
        rainProb: this.config.forecastRainProb ?? false,
      },
    };
  }
